...
### Node
```javascript
const {closest, distance, ordered} = require('fastest-levenshtein')
...
// Print string from array with lowest edit-distance to 'fast'
console.log(ordered('fast', ['slow', 'faster', 'fastest']))
//=> ['faster', 'fastest', 'slow']
```

### Deno
```javascript
import {closest, distance, ordered} from 'https://deno.land/x/fastest_levenshtein/mod.ts'
